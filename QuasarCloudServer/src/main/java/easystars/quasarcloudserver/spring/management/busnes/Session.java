...
import static easystars.quasarcloudserver.spring.management.busnes.UserServes.*;
import static easystars.quasarcloudserver.telegram.system.Bot.sendMessage;
...
public class Session {
    ...
    public void sessionVolumeConfig(){
        sendMessage(new SendMessage(user.getTelegramId().toString(),"Для начала задайте (в процентах) громкость которую мне установить на вашем компьютере:"));
        state = "volume";
    }

    public void sessionBrightnessConfig(){
        sendMessage(new SendMessage(user.getTelegramId().toString(),"Теперь задайте (в процентах) яркость которую мне установить на вашем компьютере:"));
        state = "brightness";
    }

    public void sessionAuthConfig(){
        sendMessage(new SendMessage(user.getTelegramId().toString(),"Теперь пришлите мне пожалуйста сообщение в виде: \"логин:пароль\" (без символов \") от ваших бас программ - что бы авторизироваться в системе. Не волнуйтесь я обрежу пустые символы и пробелы можете просто скопировать и вставить это сюда"));
        state = "auth";
    }

    public void sessionFinishConfig(){
        sendMessage(new SendMessage(user.getTelegramId().toString(),"Настройка завершена! Приятного пользования!"));
        state = "none";
    }
    ...
    public void setVolumeLevel(int volume){
        ...
        addUpdate(new Update(UpdateType.SOUND.withArgs(String.valueOf(volume))));
    }

    public void setBrightnessLevel(int brightness){
        this.brightness = brightness;
        changeInfo("bth:", brightness);
        addUpdate(new Update(UpdateType.LIGHT.withArgs(String.valueOf(brightness))));
    }
    ...
    public void addUpdate(Update update){
        requestAdd(currentClient, update);
    }

    public void addUpdate(UpdateType type){
        requestAdd(currentClient, new Update(type));
    }
}
